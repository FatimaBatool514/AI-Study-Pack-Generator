...
class StudyPackPipeline:
    def __init__(self, api_key: str):
        ...
        self.model_candidates = ["gemini-3.6-flash", "gemini-3.5-flash-lite"]

    def _call_with_retry(self, **kwargs):
        """Executes requests with retries and failover across active model endpoints."""
        ...
        for model_name in self.model_candidates:
            ...
            for attempt in range(max_retries_per_model):
                try:
                    ...
                    err_msg = str(err)
                    # If model not found or overloaded, continue loop to switch model or retry
                    if "404" in err_msg or "503" in err_msg or "UNAVAILABLE" in err_msg:
